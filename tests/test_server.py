...
def test_server():
    ...
    actual_tool_count = len(server.list_tools())
    if actual_tool_count == expected_tool_count:
        print(f"   ✓ Tool count correct: {actual_tool_count}")
    else:
        print(f"   ⚠ Tool count mismatch: expected {expected_tool_count}, got {actual_tool_count}")
    ...
    print("\n5. Testing health_check tool...")
    try:
        health_tool = server.get_tool("health_check")
        ...
        if result["success"]:
            ...
            if "components" in health_data:
                ...
                if "database" in components:
                    ...
                    print(
                        f"   ✓ Database: {db_health['status']} ({db_health.get('response_time_ms', 'N/A')}ms)"
                    )

                if "cache" in components:
                    cache_health = components["cache"]
                    print(
                        f"   ✓ Cache: {cache_health['status']} (hit rate: {cache_health.get('hit_rate', 'N/A')})"
                    )
        else:
    ...
    print("\n6. Testing system_info tool...")
    try:
        system_tool = server.get_tool("system_info")
        result = system_tool(include_performance=True)

        if result["success"]:
            system_data = result["data"]
            print(f"   ✓ Server version: {system_data['server']['version']}")
            print(f"   ✓ Python version: {system_data['server']['python_version']}")
            print(f"   ✓ Platform: {system_data['server']['platform']['system']}")

            if "performance" in system_data:
                perf = system_data["performance"]
                print(f"   ✓ Memory usage: {perf['memory']['percent']:.1f}%")
                print(f"   ✓ CPU usage: {perf['cpu']['percent']:.1f}%")
        else:
            print(f"   ✗ System info error: {result['error']}")
    except Exception as e:
